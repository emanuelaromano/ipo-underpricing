...
def evaluate_models(model_params, X_train, y_train, X_val, y_val):
    ...
    for i, (name, mp) in enumerate(model_params.items(), 1):
        ...
        try:
            ...
            cv_scores = cross_val_score(best_model, X_train, y_train_log, cv=5,
                            scoring='neg_root_mean_squared_error')
            cv_scores = -cv_scores

            y_pred = np.exp(best_model.predict(X_val)) - 1
            ...
            train_sizes, train_scores, val_scores = learning_curve(
                best_model, X_train, y_train_log, cv=5,
                scoring='neg_root_mean_squared_error',
...
def evaluate_best_model(model_params, X, y, top5_models_df):
    ...
    y_pred = np.exp(best_model.predict(X_test)) - 1
    y_pred_dummy = np.exp(dummy_model.predict(X_test)) - 1
