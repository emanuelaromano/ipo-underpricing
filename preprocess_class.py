...
def convert_offer_to_underpriced(input_file, output_file):
    with open(input_file, 'r', newline='', encoding='utf-8') as infile, \
        ...
        for row in reader:
            ...
            new_row.extend([trade_month, trade_day, trade_year, underpriced])
...
def show_correlation(filename,outfile):
    df = pd.read_csv(filename)
    label = df['Underpriced']
    df = df.drop(columns=['Underpriced'])
    ...
    # Ensure Underpriced is last column
    column_order = X_reduced.columns.tolist() + ['Underpriced']
    df_final = df_final[column_order]
    df_final.replace({None: 0, '': 0}, inplace=True)
...
def checkimbalance(filename):
    df = pd.read_csv(filename)
    label_count = df['Underpriced'].value_counts(normalize=True)
    print(label_count)
    print(df['Underpriced'].value_counts())

def rebalance(filename):
    df = pd.read_csv(filename)
    label = df['Underpriced']
    df = df.drop(columns=['Underpriced'])
    sampler = SMOTE(random_state=42)
    x_dev,x_test,y_dev,y_test = train_test_split(df,label,test_size=0.2,stratify=label,random_state=42)
    x_dev_samp_s, y_dev_samp_s = sampler.fit_resample(x_dev,y_dev)
    df_final = pd.concat([x_dev_samp_s,y_dev_samp_s],axis=1)
    df_final.to_csv('Balanced_Encode_Class.csv',index=False)
    print(f'the initial sampled development data has {y_dev.shape[0]} rows')
    print(f'the random over sampled development data has {y_dev_samp_s.shape[0]} rows')
    print(f'the random over sampled development set has a shape of {x_dev_samp_s.shape}')
...
if __name__ == '__main__':
    ...
    drop_column(['Issuer Ticker','Issuer Name','Filing Term Price Range','cusip','Priced Range'],'Label_Converted_data.csv','Issuer_removed_data.csv')
    ...
    show_correlation('Encoded_data.csv','Clean_Encode_Class.csv')
    checkimbalance('Clean_Encode_Class.csv')
    rebalance('Clean_Encode_Class.csv')
    pass
